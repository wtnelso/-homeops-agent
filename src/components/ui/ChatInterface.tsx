...
import { RefreshCw, AlertCircle, Sparkles, X, CheckCircle, XCircle, Users, User, Heart, Brain, ChevronLeft, ChevronRight, Cake, GraduationCap, Phone, Calendar, Mail, CalendarDays, Type, FileText, Clock, Tag, Bot } from 'lucide-react';
...
const ChatInterface: React.FC<ChatInterfaceProps> = ({
  ...
  // Check if user is in demo mode - hardcoded for reliability
  const DEMO_EMAIL = 'demo@homeops.ai';
  const isCurrentlyInDemo = userData?.user?.email === DEMO_EMAIL;
  ...
  const [pendingSuggestionsCount, setPendingSuggestionsCount] = useState<number | null>(null);
    ...
    return new Promise<void>((resolve) => {
      ...
      const typeChar = () => {
        if (index < text.length) {
          // Build string from scratch to avoid state batching issues
  ...
  // Initialize demo mode if user is demo user
  useEffect(() => {
    if (isCurrentlyInDemo) {
      demoChatService.initializeDemo();
      const demoMessages = demoChatService.getMessages();

      // Convert demo messages to ChatMessage format
      const convertedMessages: ChatMessage[] = demoMessages.map(msg => ({
        id: msg.id,
        role: msg.role,
        content: msg.content,
  ...
  useEffect(() => {
    if (suggestions.length > 0 && suggestions[currentSuggestionIndex] && !isTransitioning) {
      initializeReviewData(suggestions[currentSuggestionIndex]);
    }
  }, [currentSuggestionIndex, suggestions, isTransitioning]);
  ...
  const loadSuggestionsCount = async () => {
    if (!userData?.account?.id) {
      console.log('🔍 No user account ID available for loading suggestions count');
      return;
    }
  ...
  const enterSuggestionMode = async () => {
    ...
    // Immediately enable suggestion mode and hide header to prevent chat flash
    ...
    try {
      ...
      if (response.success) {
        ...
        // On error, exit suggestion mode and restore header
        setSuggestionMode(false);
  ...
  const exitSuggestionMode = () => {
    ...
    setTimeout(() => {
      ...
      setShowPromptsHeader(true); // Restore header when exiting
  ...
  const handleSuggestionResponse = async (suggestion: ProfileSuggestion, response: 'accept' | 'reject') => {
    ...
    try {
      ...
      if (result.success) {
        ...
        // Refresh the suggestions count since we just processed one
        loadSuggestionsCount();
  ...
  const handleSendMessage = async (messageText: string) => {
    // Wait for userData to be fully loaded before proceeding
    if (!userData || !userData.user) {
      ...
      setTimeout(() => {
        if (userData && userData.user) {
          // Try again now that userData is loaded
    ...
    if (isFirstMessage) {
      ...
      return; // Exit early to wait for animation
    }

    // For non-first messages, proceed immediately
    proceedWithMessage(messageText);
  };

  // Unified typing handler for both demo and regular modes
  const handleTypedResponse = async (aiMessage: ChatMessage, allMessages: ChatMessage[]) => {
    // Show messages without the AI response first
    const messagesWithoutLatestAi = allMessages.slice(0, -1);

    // Create the typing AI message
    const typingAiMessage: ChatMessage = {
      id: aiMessage.id,
      role: 'assistant',
      content: aiMessage.content,
      timestamp: aiMessage.timestamp,
      metadata: { ...aiMessage.metadata, typing: true }
    };

    // Set messages with typing message
    setMessages([...messagesWithoutLatestAi, typingAiMessage]);

    // Start typewriter effect
    await typewriterEffect(aiMessage.content);
  ...
  const proceedWithMessage = async (messageText: string) => {
    // Check demo mode status RIGHT NOW when message is sent (not on render)
    const isDemoMode = userData?.user?.email === DEMO_EMAIL;
    console.log('🎬 Message sending - Demo mode check at send time:', {
      isDemoMode,
    ...
    try {
      if (isDemoMode) {
        // Demo mode: use demo chat service
        console.log('🎬 Using demo chat service');
        setLoadingStatus('Thinking...');

        const result = await demoChatService.sendMessage(messageText);

        if (result.success && result.messages) {
          // Convert demo messages to ChatMessage format
          const convertedMessages: ChatMessage[] = result.messages.map(msg => ({
            id: msg.id,
            role: msg.role,
            content: msg.content,
            timestamp: msg.timestamp,
          ...
          if (lastMessage && lastMessage.role === 'assistant') {
            // Clear loading state before typing starts
            setLoading(false);
            setLoadingStatus('');
        ...
        // Early return for demo mode to prevent executing regular chat logic
        return;
      } else {
        // Regular mode: use normal chat service
        console.log('🎬 Using regular chat service (not demo mode)');
        if (!chatService) {
          setError('Chat service not available');
          return;
        }
        ...
        if (latestAiMessage) {
          ...
          // Use unified typing handler
          await handleTypedResponse(latestAiMessage, result.messages);
        } else {
      ...
      <div className="flex-1 flex flex-col w-full max-w-full overflow-x-hidden">
        ...
        >
            ...
            <div className="fixed inset-0 z-50 pointer-events-none">
              ...
              <div
                ...
                style={{
                  ...
                  animation: 'flyToHeader1 600ms ease-out forwards'
                }}
              >
                <button className="p-4 text-left bg-white border border-gray-200 rounded-xl shadow-sm">
                  <div className="flex items-center gap-3">
                    <div className="flex items-center justify-center w-8 h-8 bg-gray-100 rounded-full">
                      <Sparkles className="w-4 h-4 text-gray-600" />
                    </div>
                    <div className="text-left">
                      <h3 className="font-medium text-gray-700 text-sm leading-relaxed flex items-center gap-2">
                        Review Profile Suggestions
                        {pendingSuggestionsCount !== null && pendingSuggestionsCount > 0 && (
                      ...
                      <p className="text-xs text-gray-500">See what we've learned about you</p>
                    </div>
                  </div>
                </button>
              </div>
              ...
              {initialPrompts.slice(1, 4).map((prompt, index) => (
                <div
                  ...
                  className="absolute"
                  style={{
                    left: index < 1 ? '75%' : index === 1 ? '25%' : '75%',
                    top: index < 1 ? '60%' : '75%',
                    transform: 'translate(-50%, -50%)',
                    animation: `flyToHeader${index + 2} 600ms ease-out forwards`
                  }}
                >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 w-full">
                  ...
                  <button
                    ...
                    className={`p-4 text-left bg-white border border-gray-200 rounded-xl hover:bg-gray-50 hover:shadow-sm transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed group ${
                      loadingSuggestions ? 'scale-95 bg-gray-50' : ''
                    }`}
                  >
                    <div className="flex items-center gap-3">
                      <div className="flex items-center justify-center w-8 h-8 bg-gray-100 rounded-full">
                      ...
                      <div className="text-left">
                        <h3 className="font-medium text-gray-700 text-sm leading-relaxed group-hover:text-gray-900 flex items-center gap-2">
                          ...
                          {!loadingSuggestions && (loadingCount || pendingSuggestionsCount === null || pendingSuggestionsCount === 0) && (
                            <div className="inline-flex items-center justify-center w-5 h-5">
                              <Sparkles className="w-3 h-3 text-blue-500 animate-sparkle-twinkle" />
                            </div>
                          )}
                          {!loadingSuggestions && !loadingCount && pendingSuggestionsCount !== null && pendingSuggestionsCount > 0 && (
                            <span className="inline-flex items-center justify-center px-2 py-0.5 text-xs font-medium text-white bg-blue-500 rounded-full">
                        ...
                        {suggestions[currentSuggestionIndex].suggestion_type === 'family_info' && (
                          <div className="space-y-3">
                            ...
                            {reviewData.type && (
                              <div className="flex items-center gap-3">
                                <label className="text-sm font-bold text-gray-700 dark:text-gray-300 w-32 flex-shrink-0">
                                  <Tag className="h-4 w-4 mr-1 inline text-indigo-500" />
                                  Type:
                                </label>
                                <select
                            ...
                            {reviewData.frequency && (
                              <div className="flex items-center gap-3">
                                ...
                                <select
                                  ...
                                  onChange={(e) => setReviewData({ ...reviewData, frequency: e.target.value })}
                                  className="flex-1 text-sm border border-gray-300 dark:border-gray-600 rounded-md px-2 py-1.5 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                                >
                            ...
                            {reviewData.days && reviewData.days.length > 0 && (
                              <div className="space-y-3">
                                {/* Days picker - using flat format */}
                                <div className="flex items-center gap-3">
                                  <label className="text-sm font-bold text-gray-700 dark:text-gray-300 w-32 flex-shrink-0">
                                  ...
                                  <div className="flex flex-wrap gap-1">
                                    {(() => {
                                      ...
                                      const toggleDay = (day: string) => {
                                        const currentDays = reviewData.days || [];
                                      ...
                                      return allDays.map((day: string) => {
                                        const isSelected = reviewData.days?.includes(day);
                        ...
                        {suggestions[currentSuggestionIndex].suggestion_type === 'contact_add' && (
                          <>
                            {reviewData.name && (
                              <div className="flex items-center gap-3">
                                <label className="text-sm font-medium text-gray-700 dark:text-gray-300 w-32 flex-shrink-0">
                                  <User className="h-4 w-4 mr-1 inline text-blue-500" />
                                  Name:
                                </label>
                                <input
                                  type="text"
                            ...
                            {reviewData.phone && (
                              <div className="flex items-center gap-3">
                                <label className="text-sm font-medium text-gray-700 dark:text-gray-300 w-32 flex-shrink-0">
                                  <Phone className="h-4 w-4 mr-1 inline text-green-500" />
                                  Phone:
                                </label>
                                <input
                                  type="tel"
                            ...
                            {reviewData.email && (
                              <div className="flex items-center gap-3">
                                <label className="text-sm font-medium text-gray-700 dark:text-gray-300 w-32 flex-shrink-0">
                                  <Mail className="h-4 w-4 mr-1 inline text-red-500" />
                                  Email:
                                </label>
                                <input
                                  type="email"
                            ...
                            {reviewData.role && (
                              <div className="flex items-center gap-3">
                                <label className="text-sm font-medium text-gray-700 dark:text-gray-300 w-32 flex-shrink-0">
                                  <User className="h-4 w-4 mr-1 inline text-purple-500" />
                                  Role:
                                </label>
                                <input
                                  type="text"
          ...
          {!suggestionMode && (
            ...
            if (message.role === 'assistant' && message.metadata?.typing) {
              return (
            ...
            return (
              ...
              {loading && (
                <div className="flex gap-3 justify-start animate-fade-in">
                  <div className="flex items-center justify-center w-8 h-8 bg-blue-100 dark:bg-blue-900/20 rounded-full border border-blue-200 dark:border-blue-700">
                  ...
                  <div className="bg-gray-100 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 p-3 rounded-lg">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm text-gray-600 dark:text-gray-400">
                        {userData?.account?.agent_name || 'AI'} is {loadingStatus?.toLowerCase().replace('...', '') || 'thinking'}...
                      </span>
                    </div>
                  </div>
                </div>
