...
const IntegrationCard: React.FC<IntegrationCardProps> = ({
  ...
  const handleCopyEmail = async () => {
    if (integration.config?.email_address) {
      try {
        await navigator.clipboard.writeText(integration.config.email_address);
      ...
      <div className={`bg-white dark:bg-gray-800 rounded-2xl border p-4 sm:p-6 hover:shadow-lg transition-all duration-500 relative transform hover:scale-105 ${
        ...
        {isConnected && (
          <div className="text-center mb-3 sm:mb-4">
            ...
            {/* Show email address for gmail-forwarding */}
            {integration.id === 'gmail-forwarding' && integration.config?.email_address && (
              <div className="mt-3 px-3 py-2 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-200 dark:border-blue-800">
                <p className="text-xs font-medium text-gray-700 dark:text-gray-300 mb-2">Forward emails to:</p>
                <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 rounded-md px-2 py-2 flex items-center justify-between gap-2">
                  <code className="text-xs text-blue-600 dark:text-blue-400 font-mono font-medium break-all flex-1">
